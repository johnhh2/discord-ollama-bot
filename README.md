...
- `!slots` with a progressive jackpot, `!blackjack`, `!flip`, `!scratchoff`, and a monthly `!lottery` drawn on the 1st of each month at 6pm CT (each user's first 10 tickets per day are half price; `!lottery automatch <max>` auto-buys tickets to tie anyone who passes your total, up to your chosen cap, until the draw)
