...
- `!shop` (or `/shop`, which only you see) is one panel: pick a section — nicknames, roles, channels, fun & social, insurance, leveling, artifacts, assets, bounties — then an item; targets, roles, channels and names come from pickers and forms, and every purchase still asks you to confirm. Every typed `!shop …` form keeps working
- Buttons where typing used to be: game and AI-thread invites are Accept / Decline, the bank-heist lobby is Join / Leave / Start / Cancel, and every AI story, roleplay or RPG post carries Continue / Invite / Stop for the thread's group
- `!admin` (or `/admin`, which only you see): moderation, effects, permission overrides, coin and XP grants and counters as pickers and forms, showing only what your tier allows
