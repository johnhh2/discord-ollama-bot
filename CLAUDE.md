...
|----------|---------|-------------|
| `DISCORD_TOKEN` | _(none)_ | **Required.** Your bot token |
...
| `NSFW_API_URL` | _(none)_ | Optional; base URL for the NSFW image API (enables `!nsfw`) |
| `NSFW_API_KEY` / `NSFW_API_USER_ID` | _(none)_ | Optional; API credentials for the NSFW image endpoint |
...
**Never use an em dash (`—`).** Not in the README, not in this file, not in
code comments or docstrings, not in user-facing bot copy, and not in commit
messages. Use the punctuation the sentence actually calls for: a colon for a
definition or an explanation, a semicolon for a second independent clause,
a comma for an aside, parentheses for a parenthetical, or a full stop for a
new sentence. En dashes in numeric ranges (`100–1000`, `35–75%`) are fine,
and so are arrows (`→`).

If a sentence only holds together with a dash, it usually wants splitting in
two. Existing text was converted in September 2026, so anything still on
screen is the model to match.
...
- **Keep it concise.** Lead with the point. One to three lines is normal; about six is the ceiling, for a genuinely subtle mechanism. Give the present reason, not the story of how the code got here; git has the history. A one-clause bug history is fine when it *is* the reason the code looks odd ("save before posting, since a failed save must refund with nothing posted").
...
Always use `ZoneInfo("America/Chicago")` for CT, which handles CST/CDT automatically:
...
- **Persistent listeners (`on_raw_reaction_add`) key on state, so write that
...
3. **Revenue is counted in gameplay days, not real time.** Every claim
   ...
   property_paid_at, acquisition day)`, so no backdated rent, and a deed
...
7. **Catalog changes**: never rename a shipped `id`; keep display names
   ...
   (`user_global_display_level`, the highest level in any guild), like
...
- **Level and cost live in the catalog, not the tests.** Levels 5–50 in
  ...
  on the 🔒 markers by level, not by list index, since `!artifacts buy <n>`
...
- **The cap is per incident.** Each robbery refunds `min(loss × pct, cap)`
  on its own, with no daily or lifetime tally, no state beyond the policy row.
...
2. **End**: after the final board edit and any `announce_record` sends,
   ...
   can be edited in an archived thread, so close **last**.
...
- **Renames are rate-limited by Discord** (two name edits per channel per
  ...
  renamed at boot (`_before_rename_sweep`), since when their last rename landed
...
- **Invoker-only, and they time out** (`_OwnedView.interaction_check`).
  A prompt is a long await, so read config again after it returns rather than
...
- **Travel.** `!idle travel <place>` sets `travel_to` to any `LANDMARKS`
  ...
  no wandering, so no collision fights, and that lost income is the price,
...
- **Player fights are tuned for a handful of players, not the IRC crowd.**
  ...
  of it, for level-up and collision fights both); the house is the challenger's
...
Discord users can fire several invocations of the same command before the first finishes (spam-typing, multi-device, scripted clients). Because asyncio is cooperative, a coroutine only loses control at an `await`, so so a sequence like *check counter → await network/DB → mutate counter* is racey: two invocations both pass the check before either mutates, and the user gets the resource twice. This bot has had four of these bugs in one month (`!scratchoff`, `!daily`, `!bail`, `!shop insurance`, `!shop unoreverse`). The pattern is easy to introduce and easy to miss in code review.
