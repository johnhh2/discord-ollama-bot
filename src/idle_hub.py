...
top, a page dropdown, an action dropdown, and the last reply in a field.

Same rules as the admin panel (src/admin_hub.py): the panel never plays a
turn itself. Every pick becomes the typed subcommand — `idle shop find`,
...
The sheet page is the display, not an action: it draws `IdleCog._sheet` and
...
MAP_PAGES = frozenset({"sheet", "road"})   # the pages that carry the map
PITCH = (
    ...
    "**Join** below to start · the **Rules** page says how it works · the **Realm** page has the ladder"
)
...
class IdleItem(PanelItem):
    ...
    (`cmd_shop`); empty means the pick only redraws the panel. A form's
    ...
    the channel. `look` picks whose sheet the panel shows."""
    command: str = ""
    args: tuple = ()
...
def _sheet_items(hub: "IdleHub") -> list[IdleItem]:
    char = hub.char
    items = [IdleItem(key="refresh", label="🔄 Refresh — the sheet and the map as they are now")]
    if char is None or not char["claimed"]:
        items.append(IdleItem(
            key="join", command="cmd_join", title="Join the Idle RPG", fields=(_class_field(),),
    ...
    if char is not None:
        items.append(IdleItem(key="items", label="🎒 Items — your items, bag and potions", command="cmd_items"))
    if hub.target != hub.ctx.author.id:
        items.append(IdleItem(key="own", label="↩️ Back to your own sheet", look=True))
    items.append(IdleItem(key="look", label="🔎 Look at a player — their sheet and the map", title="Look at a player",
                          fields=(_USER,), look=True))
    items.append(IdleItem(key="their-items", label="🎒 A player's items", title="A player's items", command="cmd_items",
                          fields=(_USER,), call=lambda hub, v: ((), {"member": _picked_user(v)})))
    return items


def _town_items(hub: "IdleHub") -> list[IdleItem]:
    char = hub.char
    if char is None:
        return []
    items = []
    if rpg.market_in_reach(char) is not None:
        for key, text in hub.cog._shop_lines(char):
            ...
            base = dict(key=f"shop-{key}", label=f"🛒 {label}", description=price, command="cmd_shop")
            if key == "potion":
        ...
        if char["gold"] > 0:
            items.append(IdleItem(
                key="gamble", label="🎲 The tables — bet gold at even money", command="cmd_gamble", title="The Tables",
        ...
        if not rpg.hunting(char) and not rpg.hunt_wait(char, int(time.time())):
            items.append(IdleItem(key="hunt", label="🏹 Hunt — take one off the town's board", command="cmd_hunt",
                                  description="kill N of a kind for a slice off your clock"))
    on = bool(char["auto_trade"])
    items.append(IdleItem(
        key="auto", label=f"🔁 Auto-trade — turn {'off' if on else 'on'}", command="cmd_shop", args=("auto",),
        kwargs={"arg": "off" if on else "on"},
        description="in a town's centre your character shops on its own" if on else "your character keeps its gold in town",
    ))
    return items


def _road_items(hub: "IdleHub") -> list[IdleItem]:
    char = hub.char
    items = []
    if char is not None:
        rpg.ensure_position(char, hub.cog.rng)   # what `!idle travel` does first
        quest = hub.cog._quest(hub.ctx.guild.id)
        if not (quest.get("kind") == "journey" and hub.ctx.author.id in quest["members"]):
            places = tuple(
            ...
            items.append(IdleItem(
                key="travel", label="🧭 Travel — walk to a town or a wild place", command="cmd_travel", title="Travel",
            ...
            if char.get("travel_to"):
                items.append(IdleItem(key="stop", label=f"🛑 Stop — give up on {char['travel_to']} and wander again",
                                      command="cmd_travel", kwargs={"where": "stop"}))
    items.append(IdleItem(
        key="lore", label="📜 Lore — read about a place", command="cmd_lore", title="Lore",
...
def _character_items(hub: "IdleHub") -> list[IdleItem]:
    char = hub.char
    if char is None:
        return []
    items = [
        IdleItem(
            key="align", label="⚖️ Alignment — a law and a moral, once a day", command="cmd_align", title="Alignment",
        ...
        IdleItem(key="titles", label="🎖️ Titles — what you've earned and what's left", command="cmd_title"),
    ]
    earned = char.get("titles") or []
    if earned:
        options = tuple((rpg.TITLE_NAMES[key], key) for key in earned) + (("None — go by your own name", "none"),)
        items.append(IdleItem(key="wear", label="🎖️ Wear a title", command="cmd_title", title="Wear a title",
                              fields=(Field("title", "Title", kind="choices", options=options),),
                              call=lambda hub, v: ((), {"which": v["title"][0]})))
    items.append(IdleItem(
        key="duel", label="🤺 Duel — challenge a player, once a day", command="cmd_duel", title="Duel", public=True,
    ...
    if char["level"] >= rpg.PRESTIGE_LEVEL:
        items.append(IdleItem(key="prestige", label="🌟 Prestige — start over at level 0 for a ★", command="cmd_prestige",
                              description="confirm in the channel", public=True))
    items.append(IdleItem(key="leave", label="🪦 Retire — delete your character for good", command="cmd_leave",
                          description="confirm in the channel", public=True))
    return items


def _realm_items(hub: "IdleHub") -> list[IdleItem]:
    items = [
        IdleItem(key="world", label="🌍 The realm today — events, blessings and your boost", command="cmd_world"),
        IdleItem(key="quest", label="📜 Quest — the running quest, or what the next one needs", command="cmd_quest"),
        IdleItem(key="top", label="🏔️ Ladder — this server's top adventurers", command="cmd_top"),
    ]
    if hub.char is not None:
        items.append(IdleItem(key="bless", label="🕊️ Bless the realm — an hour's speed for everyone here",
                              description=f"{rpg.BLESS_COST:,} of your gold · confirm in the channel",
                              command="cmd_bless", public=True))
    return items


def _rules_items(hub: "IdleHub") -> list[IdleItem]:
    from src.cogs.idle_cog import _RULES_TOPICS
    return [IdleItem(key=f"rules-{topic}", label=f"📖 {topic.title()}", command="cmd_rules", args=(topic,)) for topic in _RULES_TOPICS]


def _admin_items(hub: "IdleHub") -> list[IdleItem]:
    return [
        IdleItem(key="hog", label="⚡ Hand of God — strike a character now", command="cmd_admin_hog", title="Hand of God",
                 fields=(_USER,), call=lambda hub, v: ((), {"who": _picked_user(v)})),
        IdleItem(key="gold", label="💰 Gold — give some, or take it with a leading −", command="cmd_admin_gold", title="Gold",
                 fields=(_USER, Field("amount", "Amount", placeholder="500, or -500", max_length=12)),
                 call=lambda hub, v: ((_picked_user(v), v["amount"]), {})),
        IdleItem(key="push", label="⏱️ Push — move a character's clock", command="cmd_admin_push", title="Push a clock",
                 description="−2h brings the next level sooner, +1d later",
                 fields=(_USER, Field("amount", "Time", placeholder="-2h, +1d", max_length=12)),
                 call=lambda hub, v: ((_picked_user(v), v["amount"]), {})),
        IdleItem(key="move", label="📍 Move — set a character down somewhere", command="cmd_admin_move", title="Move a character",
        ...
        IdleItem(key="remove", label="🗑️ Remove — delete a player's character", command="cmd_admin_remove", title="Remove a character",
                 description="confirm in the channel", fields=(_USER,), public=True,
                 call=lambda hub, v: ((), {"who": _picked_user(v)})),
        IdleItem(key="reset", label="💥 Reset — wipe this server's game", command="cmd_admin_reset",
                 description="confirm in the channel", public=True),
    ]


_BUILDERS = {
    "sheet": _sheet_items, "town": _town_items, "road": _road_items, "character": _character_items,
    "realm": _realm_items, "rules": _rules_items, "admin": _admin_items,
}
...
def pages_for(hub: "IdleHub") -> list[tuple[str, str]]:
    """The sheet, the realm and the rules for everyone; the town, the road
...
class IdleHub(Panel):
    ...
    def __init__(self, cog, ctx, page: str = "sheet"):
        self.cog = cog
        self.target = ctx.author.id   # whose sheet the sheet page shows
    ...
    def embed(self) -> discord.Embed:
        ...
        embed.set_footer(text="Pick a page, then an action. Typed commands keep working: !idle status, !travel, !store…")
        return embed
    ...
    async def attachments(self) -> list[discord.File]:
        ...
        shown = self.target if self.page == "sheet" else self.ctx.author.id
