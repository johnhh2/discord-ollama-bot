...
The closest we get to "does the bot boot?" without a gateway connection:
build a real `commands.Bot` via `create_bot()`, run every cog's `setup()`
through `_load_extensions(bot)`, then assert the expected commands are
registered.

What this catches that direct cog-instantiation tests miss:
- CommandRegistrationError: two cogs — or two declarative aliases inside one
  cog — claiming the same command name. ShopCog's _SHOP_TOP_ALIASES alone
  has 25 names.
- A cog file that fails to import (NameError, missing module).
- decorator-order regressions on @requires_perm: discord.py's command
  introspection reads `__wrapped__` to resolve converters, so a broken
  order fails register-time validation.
"""
import pytest
...
async def loaded_bot():
    ...
    `Bot()` opens no network connection — only `.run()` / `.start()` does —
    so constructing one and loading extensions is safe in a test.

    `load_extension('src.X')` rebinds `sys.modules['src.X']` to a freshly
    imported module object: anything that imported src.X at collection time
    (e.g. `import src.events as _events` in test_schedulers.py) keeps the old
    reference, and module-private state such as
    src.events._SOUNDBOARD_TIMESTAMPS ends up bifurcated. Hence the
    snapshot-and-restore below.
    """
    import sys
...
async def test_bot_loads_every_named_extension(loaded_bot):
    """`bot.extensions` must be exactly the dotted names in EXTENSIONS, which
    pins additions to the list. A module that can't import or has no setup()
    already raised inside the fixture."""
...
async def test_no_duplicate_top_level_command_names(loaded_bot):
    ...
    during the second add_command and the fixture fails. This makes that
...
async def test_slash_ask_is_registered_in_the_command_tree(loaded_bot):
    ...
    assert {"ask", "settings"} <= tree_names, f"app commands: {sorted(tree_names)}"
